...
{
    //Using static keyword so we can get access to InitDb method without having to initialise a new class of DbInitializer
    public static void InitDb(WebApplication app)
    {
        // This scope gives us access to services from the program.cs class by creating an instance of IServiceScope
        // We use the Using keyword as it implements an IDisposable Interface which implements IDisposable, it ensures that the Dispose method of that object is called when the block of code is exited. 
        // This is particularly useful for scenarios where resources need to be explicitly released or cleaned up.
        using var scope = app.Services.CreateScope();

        // Now that we have access to the services container we can use the scope to get the AuctionDbContext
